...
{
    /**
     * Listado de Casos Activos (Pendientes o En Gestión)
     */
    public function index(Request $request): View
    {
        $query = Caso::query();

        // --- FILTRO DE ESTADO BASE ---
        // La Encargada NO ve los casos terminados en esta vista principal.
        $query->whereNotIn('estado', ['Finalizado', 'Rechazado', 'Aceptado']);

        // Aplicamos los filtros dinámicos del formulario
        $this->aplicarFiltros($query, $request);
        
        // Ordenamiento dinámico (si viene en la URL) o por defecto latest()
        if ($request->has('sort')) {
    ...
    /**
     * Función auxiliar para aplicar los filtros comunes (Index y Finalizados)
     */
    private function aplicarFiltros($query, Request $request)
    {
        // 1. Filtro RUT (Ahora busca RUT o Nombre para ser más flexible)
        ...
        // 2. Filtro Estado Específico (Seleccionable en el dropdown)
        if ($request->filled('search_estado')) {
            $query->where('estado', $request->input('search_estado'));
        }

        // 3. Filtro Fechas (CORREGIDO: Usamos los nombres estandarizados 'search_')
    ...
    /**
     * Muestra el historial de casos finalizados.
     */
    public function finalizados(Request $request)
    {
        // 1. Base Query: Solo casos terminados
        $query = Caso::with('estudiante')
                     ->whereIn('estado', ['Finalizado', 'Aceptado', 'Rechazado']);

        // 2. Filtro: Buscador General (RUT o Nombre)
        if ($request->filled('search_rut')) {
            $busqueda = $request->input('search_rut');
            $query->where(function($q) use ($busqueda) {
                $q->where('rut_estudiante', 'like', "%$busqueda%")
        ...
        // 3. Filtro: Rango de Fechas (CORREGIDO: Usamos search_fecha...)
        // Filtramos por 'updated_at' porque es la fecha de cierre
        if ($request->filled('search_fecha_inicio')) {
            $query->whereDate('updated_at', '>=', $request->input('search_fecha_inicio'));
        }
        if ($request->filled('search_fecha_fin')) {
            $query->whereDate('updated_at', '<=', $request->input('search_fecha_fin'));
        }
        ...
        // 5. Paginación con persistencia de filtros
        $casos = $query->paginate(10)->appends($request->all());

        return view('encargada-inclusion.casos.finalizados', compact('casos'));
    }
}